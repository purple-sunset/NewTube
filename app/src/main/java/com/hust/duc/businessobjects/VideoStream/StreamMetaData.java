...
//	}
	...
	private void setMediaFormat(int itag) {
		...
		if(this.format == MediaFormat.UNKNOWN) {
			if(this.uri.toString().contains("mime=video%2Fmp4"))
				this.format = MediaFormat.MPEG_4;
			else if(this.uri.toString().contains("mime=video%2Fwebm"))
				this.format = MediaFormat.WEBM;
			else if(this.uri.toString().contains("mime=video%2F3gp"))
				this.format = MediaFormat.V3GPP;
		}
	}
