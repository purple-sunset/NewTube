...
import com.hust.duc.businessobjects.db.SubscriptionsDb;
import com.hust.duc.gui.app.NewTubeApp;
import com.hust.duc.gui.businessobjects.SubscriptionsFragmentListener;

import static com.hust.duc.gui.app.NewTubeApp.getContext;
	...
	private class GetChannelVideosTask extends AsyncTaskParallel<Void, Void, List<YouTubeVideo>> {
		...
		public GetChannelVideosTask(YouTubeChannel channel) {
			try {
				 ...
				 * any.
 				 */
				long l = NewTubeApp.getPreferenceManager().getLong(NewTubeApp.KEY_SUBSCRIPTIONS_LAST_UPDATED, -1);
				if(l != -1) {
		...
		protected void onPostExecute(List<YouTubeVideo> youTubeVideos) {
			...
			if(numTasksFinished < numTasksLeft) {
				...
				SharedPreferences.Editor editor = NewTubeApp.getPreferenceManager().edit();
				editor.putLong(NewTubeApp.KEY_SUBSCRIPTIONS_LAST_UPDATED, new DateTime(new Date()).getValue());
				editor.commit();
			}
			if(listener != null)
